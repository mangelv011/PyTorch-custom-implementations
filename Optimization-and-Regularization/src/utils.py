...
class StepLR(torch.optim.lr_scheduler.LRScheduler):
    ...
    def __init__(
        ...
        # Guardar el tamaño del paso (número de pasos para reducir la tasa de aprendizaje)
        self.step_size = step_size
        # Guardar el factor de reducción de la tasa de aprendizaje
        self.gamma = gamma
        # Inicializar el contador de épocas
        self.last_epoch = -1
    ...
    def step(self, epoch: Optional[int] = None) -> None:
        ...
        # Comprobar si es necesario reducir la tasa de aprendizaje
        if self.last_epoch > 0 and self.last_epoch % self.step_size == 0:
            # Recorrer todos los grupos de parámetros del optimizador
            for param_group in self.optimizer.param_groups:
                # Reducir la tasa de aprendizaje multiplicándola por el factor gamma
                param_group['lr'] = param_group['lr'] * self.gamma


def get_dropout_random_indexes(shape: List[int], p: float) -> torch.Tensor:
    """
