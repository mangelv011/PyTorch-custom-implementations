...
def main() -> None:
    ...
    train_data: DataLoader
    val_data: DataLoader
    train_data, val_data, _ = load_data(DATA_PATH, batch_size=batch_size, num_workers=dataloader_workers)

    # define name and writer
    name: str = f"model_lr_{lr}_pretrained_{unfreezed_layers}_{batch_size}_{epochs}"
    writer: SummaryWriter = SummaryWriter(f"runs/{name}")

    # define model and move it to device immediately
    model: torch.nn.Module = CNNModel(
        num_classes=NUMBER_OF_CLASSES, 
        freeze_base=True, 
        unfreeze_layers=unfreezed_layers
